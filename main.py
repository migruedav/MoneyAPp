...
async def home():
    ...
    cuentas = {'BanortePlatinum': 'Banorte Platinum', 'BanorteClasica': 'Banorte Clásica', 'Efectivo': 'Efectivo',
               'Banamex': 'Banamex', 'BanamexRewards': 'Banamex Rewards', 'Nu': 'Nu', 'BBVA': 'BBVA'}

    TotalPorCuenta = {}
    for k, v in cuentas.items():
        total = (sum([i['monto'] for i in movimientos if (i['tipo'] == 'Ingreso') and (i['cuenta'] == v)]))-(sum([i['monto'] for i in movimientos if (i['tipo'] == 'Egreso') and (i['cuenta'] == v)]))+(sum([i['monto']
                                                                                                                                                                                                             for i in movimientos if ('transcuentaingreso' in i) and (i['transcuentaingreso'] == v)]))-(sum([i['monto'] for i in movimientos if ('transcuentaegreso' in i) and (i['transcuentaegreso'] == v)]))
        TotalPorCuenta[k] = total

    TotalIngresos = sum([i['monto'] for i in movimientos if (
        i['tipo'] == 'Ingreso') and (i['subcategoria'] != 'Transferencia')])
    TotalEgresos = sum([i['monto'] for i in movimientos if (
        i['tipo'] == 'Egreso') and (i['subcategoria'] != 'Transferencia')])
    GranTotal = TotalIngresos-TotalEgresos

    db.collection('home').document('d7x71RhHQede3VxMuMgN').set(TotalPorCuenta)
    db.collection('home').document('d7x71RhHQede3VxMuMgN').set(
        {'TotalIngresos': TotalIngresos, 'TotalEgresos': TotalEgresos, 'GranTotal': GranTotal}, merge=True)

    return "Datos del Home actualizados"


@app.get("/gastado")
async def get_gastado():
    def gastado():
...
async def ingresado():
    ...
    for i in docs:
        if i.to_dict()['tipo'] == 'Ingreso':
            if i.to_dict()['subcategoria'] in ingresos_por_subcategoria:
                ingresos_por_subcategoria[i.to_dict(
                )['subcategoria']] += i.to_dict()['monto']
            else:
                ingresos_por_subcategoria[i.to_dict()['subcategoria']] = i.to_dict()[
                    'monto']
    ...
    for k, v in ingresos_por_subcategoria.items():
        for doc in docs:
            if doc.to_dict()['subcategoria'] == k:
                doc_ref = db.collection(
                    'subcategoriasIngresos').document(doc.id)
...
async def mensualidad(nombre: str = Query(), mesdemensualidad: str = Query()):
    ...
    for i in alumnos:
        if i['nombre'] == nombre:
            ...
            db.collection('alumnos').document(i['doc_id']).set(
                {'mensualidades': listamensualidades}, merge=True)
